...
5. **Core extension architecture is solid.** RuntimeControlExtension uses the same contract as external extensions — good dogfooding.
...
**Status:** Completed in `CopilotRuntimeClient.CycleSessionsAsync` (hooks fire before `_sessions.Clear()`).
...
**Status:** Completed in `ExtensionManager.ResolveExternalLoadOrder` + `HasPermanentDependencyFailure` using resolved-or-core IDs.
...
**Status:** Completed. `ReloadExternalAsync` now cycles sessions; `/reload` no longer does a separate cycle.
...
`RuntimeControlExtension` was appended to the large ExtensionManager file. It is a separate concern.

**Fix:** Move to `Core/Extensions/RuntimeControlExtension.cs`.
**Status:** Completed. (`MindReaderExtension` was removed to rely on Copilot CLI built-in file tools.)
...
**Status:** Verified current `docs/msclaw-walkthrough.md` contains a single heading block; no extra edit required.
...
**Status:** Completed in `ScanManifestsInto` via immediate subdirectory scan.
...
**Status:** Completed by replacing blocking shutdown registration with `await app.RunAsync()` in `try/finally` and async `ShutdownAsync()`.

---

## Summary

The implementation faithfully covers the spec's core requirements. Extension contract, discovery, loading, hooks, commands, and error isolation are well-built.

Resolved in this pass: **#1, #2, #3/#4, #7, #8 (verified), #9, #10**.

Remaining intentional gap:

- **Lockfile/CLI commands (#5)** — spec'd but deferred; roadmap callout remains the right treatment for this phase.
