...
| 3.0 | 2026-03-04 | MsClaw Team | Renumbered epics in priority order (implementation sequence). Merged Graceful Shutdown into Gateway Protocol as a single epic. 10 epics → 9. |
| 3.1 | 2026-03-04 | MsClaw Team | Aligned with gateway-mind.md v2.0 two-layer model: library primitives vs agent runtime behavior. Clarified agent writes to mind using its own Copilot CLI tools, not library-provided write operations. |
...
The gateway hosts a single **mind** — a directory on disk that is the agent's entire memory, personality, and knowledge base. The mind is not a configuration file; it is a living, structured body of knowledge that the agent reads at the start of every session and writes to as it learns. The library (MsClaw.Core) provides read-only primitives — scaffolding, validation, identity assembly, and path-protected file reading. The agent writes to the mind using its own Copilot CLI filesystem tools (read, write, create, move, delete), guided by the personality and instructions loaded from the mind. Everything the agent knows lives here. The full mind system is specified in [gateway-mind.md](gateway-mind.md).
...
- **Working memory files missing:** IF `memory.md`, `log.md`, or `rules.md` do not exist when the agent reads them, the agent MUST proceed without error. The agent MUST create missing files on first write using its own filesystem tools. See [gateway-mind.md](gateway-mind.md).
