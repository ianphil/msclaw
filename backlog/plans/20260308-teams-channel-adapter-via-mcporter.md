...
| Q2 | Tool count limits | Lazy tool loading via meta-tools | Session starts with `tool_registry_search` and `tool_registry_load`. Agent discovers and loads only the tools it needs. `tool_registry_load` resumes the session via `ResumeSessionAsync` with the expanded tool set (preserves conversation history). |
